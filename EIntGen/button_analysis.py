...
def exportFeedback_wfactors():
	csvfileout = open('all_userbutton.csv', 'w')
	csvheader=("idx","datetime","child_id","child_name","session","fformation","alone","groupid","gender","chouchou","button")
		...
		for row in data:
			...
			for i in info:
				...
				if(current_time>= start_time and  current_time<= end_time):
					button = row[-1]
					new_row = {"idx":row[0],"datetime":datetime.strftime(current_time,'%Y-%m-%d %H:%M:%S:%f'),"child_id":i[0],"child_name":i[1],"session":i[-5],"fformation":i[5],"alone":i[-4],"groupid":i[-3],"gender":i[-2],"chouchou":i[-1],"button":button}
